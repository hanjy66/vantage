...
DUCKDUCKGO_SEARCH_DESCRIPTION = (
    "A search engine that returns results from DuckDuckGo. "
    "Useful for answering questions about current events and general knowledge."
)
    ...
    with DDGS() as ddgs:
        for query in queries:
            try:
                ...
                all_results.append(f"Error searching for '{query}': {e}")
...
async def tavily_search(
    ...
    summarization_model = init_chat_model(**_summ_kwargs).with_structured_output(Summary, method="function_calling").with_retry(
...
async def get_search_tool(search_api: SearchAPI):
    ...
    if search_api == SearchAPI.ANTHROPIC:
        ...
        duckduckgo_search.metadata = {
            **(duckduckgo_search.metadata or {}),
            "type": "search",
            "name": "web_search"
        }
...
async def get_all_tools(config: RunnableConfig):
    ...
    # 非侵入追加：中文源（智谱）+ 面经（仅面试模式）——让 researcher 模型自行决定何时调用。
    # 主图不改，仅在此装配工具列表。search_api == NONE（用户显式禁用搜索）时不追加。
    if search_api != SearchAPI.NONE:
        ...
        if get_zhipu_api_key(config):
            tools.append(zhipu_search)
        # 权威源深度检索：有 Tavily key 即挂，两个模式通用（深度技术在 general 模式也常见），由模型自选。
        if get_tavily_api_key(config):
            tools.append(authoritative_search)
        try:
            from src.modes.router import resolve_mode
            if resolve_mode(config).mode == "interview":
                tools.append(search_interview_experience)
        except Exception:
            pass
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    ...
    if should_get_from_config.lower() == "true":
        ...
        # openai:gemini-* → 走 Gemini 代理 key（与 DeepSeek key 分离）
        if model_name_lower.startswith("openai:gemini"):
            return os.getenv("GEMINI_PROXY_KEY") or os.getenv("OPENAI_API_KEY")
        # openai:kimi-* / openai:moonshot-* → 走 Moonshot key（与 DeepSeek key 分离）
        elif model_name_lower.startswith("openai:kimi") or model_name_lower.startswith("openai:moonshot"):
...
def get_base_url_for_model(model_name: str) -> str | None:
    """Return proxy base URL for models that need a non-default endpoint.
    openai:gemini-*  → GEMINI_PROXY_URL（与 DeepSeek OPENAI_BASE_URL 分离）。
    openai:kimi-*    → MOONSHOT_BASE_URL（与 DeepSeek OPENAI_BASE_URL 分离）。
    """
    name = model_name.lower()
    if name.startswith("openai:gemini"):
        return os.getenv("GEMINI_PROXY_URL")
    if name.startswith("openai:kimi") or name.startswith("openai:moonshot"):
